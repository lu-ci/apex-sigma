...
`>>blackjack`  |  Starts a game of Blackjack for you on the current server. You can specify the amount you want to bet on the game. Use bjdraw to draw a card. Use bjfold to fold and hope the dealer busts. And you can use bjquit to quit the game, in which case your buyin will be refunded.  |  `>>blackjack 5000`
`>>bjdraw`  |  Draws the next card for your blackjack game.  |  `>>bjdraw`
`>>bjfold`  |  Fold and see the end results of the dealer.  |  `>>bjfold`
`>>bjquit`  |  Quits your current blackjack game.  |  `>>bjquit`
