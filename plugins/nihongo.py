...
class WK(Plugin):
    ...
    async def on_message(self, message, pfx):
        if message.content.startswith(pfx + cmd_wk):
            ...
            try: self.log.info('User %s [%s] on server %s [%s], used the ' + cmd_name + ' command on #%s channel',
                              message.author,
                              message.author.id, message.server.name, message.server.id, message.channel)
            except: self.log.info('User %s [%s], used the ' + cmd_name + ' command.',
                              message.author,
                              message.author.id)

            if len(message.mentions) == 0:  #no mentions in the message
                if message.content == (pfx + cmd_wk): user_id = str(message.author.id)  #if no arguments passed, pulling the ID of a caller
                else:
                    key = None
                    username = message.content[len(pfx) + len(cmd_wk) + 1:]  #otherwise, pull the username out of a message
            else: #if there are mentions in the message
                try: user_id = message.mentions[0].id  #pull the mentioned ID
                except:
            ...
            if key != None:  #if we have a key, pull data directly from API
                try:
                    ...
                    try: next_review_date = bold(datetime.datetime.fromtimestamp(api3['requested_information']['next_review_date']).strftime('%B %d, %Y %H:%M'))
                    except TypeError: pass  #NoneType on retrival, user is on vacation
                    ...
                    if api3['requested_information']['reviews_available'] > 150: warning = ':exclamation:'
                    else: warning = ''
                ...
                try: script = tree.xpath('//div[@class="footer-adjustment"]/script')[0].text.strip()
                except IndexError:
                    await self.client.send_message(message.channel, "Error while parsing the page, profile not found or doesn't exist")
                    return
                script = script[script.find('var srsCounts') : script.find('Counts.fillInSrsCount(srsCounts.requested_information);')]
            ...
            try:
                ...
                if 'api3' in locals():
                    ...
                    except UnboundLocalError: pass #no review date, user is on vacation
                ...
                await self.client.send_message(message.channel, 'Something went wrong ¯\_(ツ)_/¯. Error while parsing the data')
                return
    ...
    async def on_message(self, message, pfx):
        if message.content.startswith(pfx + cmd_wk_store):
            ...
            try:
                ...
                if mode == '':
                    await self.client.send_message(message.channel, 'Bind your Discord profile and your API key or username\n'
            ...
            except: await self.client.send_message(message.channel, 'Error while parsing the input message')
