...
from config import cmd_anime, cmd_manga
...
class MAL(Plugin):
    ...
    async def on_message(self, message, pfx):
        ...
        elif message.content.startswith(pfx + cmd_manga):
            await self.client.send_typing(message.channel)
            cmd_name = 'MyAnimeList Manga Lookup'
            self.log.info('\nUser %s [%s] on server %s [%s], used the ' + cmd_name + ' command on #%s channel',
                          message.author,
                          message.author.id, message.server.name, message.server.id, message.channel)
            mal_input = message.content[len(pfx) + len(cmd_manga) + 1:]
