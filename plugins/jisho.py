...
class Jisho(Plugin):
    ...
    async def on_message(self, message, pfx):
        if message.content.startswith(pfx + cmd_jisho):
            ...
            if request['data']: request = request['data'][0]
            else:
            ...
            try: output += "{} 【{}】".format(request['japanese'][0]['word'], request['japanese'][0]['reading'])
            except KeyError: output += "{}".format(request['japanese'][0]['reading'])

            # parsing tags
            if request['is_common']: output += ' | common word'
            wk_lvls = []
            for tag in request['tags']:
                if tag.find('wanikani') != -1: wk_lvls.append(tag[8:])
            if wk_lvls: output += ' | Wanikani level {}'.format(', '.join(wk_lvls))

            definitons_len = 1
            if len(request['senses']) > 5: definitons_len = 5
            else: definitons_len = len(request['senses'])

            for i in range(0, definitons_len):
                output += '\n'
                etc = []
                output += '{}. {}'.format(i + 1, '; '.join(request['senses'][i]['english_definitions']))
                if request['senses'][i]['parts_of_speech']: etc.append(', '.join(request['senses'][i]['parts_of_speech']))
                if request['senses'][i]['tags']: etc.append('; '.join(request['senses'][i]['tags']))
                if request['senses'][i]['see_also']: etc.append('See also {}'.format(', '.join(request['senses'][i]['see_also'])))
                if request['senses'][i]['info']: etc.append('; '.join(request['senses'][i]['info']))
                if etc: # attaching definition tags
                    if etc[0]: output += '\n| ' + ', '.join(etc)
                etc += '\n'
