...
async def markov(cmd, message, args):
    collect = cmd.db.get_settings(message.guild.id, 'MarkovCollect')
    if collect:
        directory = 'chains/'
        filename = f'chain_{message.guild.id}.yml'
        ...
        if os.path.exists(chain_location):
            with open(chain_location) as chain_file:
                ...
                response.add_field(name=f':link: {message.guild.name} Markov Chain Response',
                                   value=f'```\n{output}\n```')
        else:
            response = discord.Embed(color=0xDB0000, title='⛔ No Chain File Was Found')
    else:
        response = discord.Embed(color=0xDB0000,
                                 title=f'⛔ Markov Collection Is Disabled on {message.guild.name}')
