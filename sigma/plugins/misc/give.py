async def give(cmd, message, args):
    ...
    if amount > curr_points:
        await cmd.bot.send_message(message.channel, 'You don\'t seem have enough points, <@' + message.author.id + '>...')
        return
    else:
        cmd.db.take_points(message.server, message.author, amount)
        cmd.db.add_points(message.server, target_user, amount)
        await cmd.bot.send_message(message.channel, 'Alright <@' + message.author.id + '>, I\'ve transfered **' + str(amount) + '** points to <@' + target_user.id + '>!')
