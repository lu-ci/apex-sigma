...
async def overwatch(cmd, message, args):
    ...
    if ow_region == 'us' or 'eu':
        ...
        base.save(f'cache/ow_profile_{message.author.id}.png')
        ...
        await message.channel.send(file=discord.File(f'cache/ow_profile_{message.author.id}.png'))
        os.remove(f'cache/ow_profile_{message.author.id}.png')
