...
async def wffissurechannel(cmd, message, args):
    if check_admin(message.author, message.channel):
        if message.channel_mentions:
            ...
            if args:
                if args[0].lower() == 'disable':
                    ...
                    response = discord.Embed(title=f'✅ Warframe Void Fissure Channel Disabled')
        ...
        response = discord.Embed(title=f'✅ Warframe Void Fissure Channel set to #{target_channel.name}')
    else:
