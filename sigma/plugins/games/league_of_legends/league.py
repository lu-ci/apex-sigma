...
async def league(cmd, message, args):
    ...
    base.save(f'cache/lol_profile_{message.author.id}.png')
    # Image End
    ...
    if ranked_text == 'None' and normal_text == 'None':
        ...
        await message.channel.send(file=discord.File(f'cache/lol_profile_{message.author.id}.png'))
        await message.channel.send('Normal Stats:\n```' + normal_text + '\n```\nRanked Stats:\n```' + ranked_text + '\n```')
        os.remove(f'cache/lol_profile_{message.author.id}.png')
