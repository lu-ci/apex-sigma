...
async def greetmsg(cmd, message, args):
    if message.server is not None:
        ...
        if admin_check is True:
            ...
            if existance == '0':
                await cmd.bot.send_message(message.channel, 'No greet settings exist for this server.')
            else:
                if args:
                    greet_msg = ' '.join(args)
                    ...
                    cmd.db.commit()
                    await cmd.bot.send_message(message.channel, '**New Greet Message Set**')
                else:
                    greet_msg = ''
                    greet_grab = cmd.db.execute(msg_query, message.server.id)
                    for result in greet_grab:
                        greet_msg = result[0]
                    await cmd.bot.send_message(message.channel, '**Current greeting message is:**\n```\n' + greet_msg + '\n```')
    else:
        response = await cmd.bot.send_message(message.channel, 'Only an **Administrator** can manage the greet message. :x:')
