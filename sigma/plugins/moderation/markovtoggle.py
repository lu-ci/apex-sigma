...
async def markovtoggle(cmd, message, args):
    if check_admin(message.author, message.channel):
        active = cmd.db.get_settings(message.guild.id, 'MarkovCollect')
        if active:
            cmd.db.set_settings(message.guild.id, 'MarkovCollect', False)
            out_content = discord.Embed(type='rich', color=0x66CC66,
                                        title='✅ Markov Collection Disabled')
        else:
            cmd.db.set_settings(message.guild.id, 'MarkovCollect', True)
            out_content = discord.Embed(type='rich', color=0x66CC66,
